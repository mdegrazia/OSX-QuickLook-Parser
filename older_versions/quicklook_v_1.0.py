...
with con:	
	...
	sql = "select distinct k.folder,k.file_name,k.version,t.hit_count,t.last_hit_date, datetime(t.last_hit_date + strftime('%s', '2001-01-01 00:00:00'), 'unixepoch') As [decoded-last_hit_date] from (select rowid as f_rowid,folder,file_name,version from files) k left join thumbnails t on t.file_id = k.f_rowid order by t.hit_count DESC"
	cur.execute(sql)
	try:
		cur.execute(sql)
	except:
	...
	print "Folder\tFilename\tHit Count\tLast Hit Date\tDecoded Hit Date (UTC)\tVersion"
	...
	for row in rows:
		...
		print row[0].encode('ascii','ignore') + "\t" + row[1].encode('ascii','ignore') + "\t" + str(row[3]) + "\t" + str(row[4]) + "\t" + str(row[5]) + "\t" + str(version_string)
